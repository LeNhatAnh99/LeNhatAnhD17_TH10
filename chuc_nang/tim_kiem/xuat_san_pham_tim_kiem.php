<?php 
	...
	{ 
		...
		for($i=0;$i<count($m);$i++)
		{
			...
			if($tu!="")
			{
				$chuoi_tim_sql=$chuoi_tim_sql." ten like '%".$tu."%' or";//% là hiep ttim ben trai hay
			}
		}

		$m_2=explode(" ",$chuoi_tim_sql);    ////loi sua lai
		$chuoi_tim_sql_2="";
		...
		$so_du_lieu=15;
		$sql="select count(*) from san_pham  where $chuoi_tim_sql_2";
		$tam=$conn->query($sql);
		$data=$tam->fetch();
		...
		$sql="select id,ten,gia,hinh_anh,thuoc_menu from san_pham where $chuoi_tim_sql_2 order by id desc limit $vtbd,$so_du_lieu";
