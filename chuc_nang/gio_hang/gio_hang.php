	...
	else 
	{
		...
		echo "<table>";
			...
			for($i=0;$i<count($_SESSION['id_them_vao_gio']);$i++)
			{
				...
				$sql="select id,ten,hinh_anh,gia from san_pham where id='".$_SESSION['id_them_vao_gio'][$i]."'";//ANh
				$tam=$conn->query($sql);
				$data=$tam->fetch();
				
				$tien=$data['gia']*$_SESSION['sl_them_vao_gio'][$i];
				...
				{
				echo "<tr>";
					echo "<td>"."<img src='$link_anh' width='60px'>"."</td>";//Them Hinh Anh
